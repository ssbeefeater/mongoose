 ...
 *     var minlength = [5, function(){ return 'The value of path `{PATH}` (`{VALUE}`) is shorter than the minimum allowed length ({MINLENGTH}).'}];
 ...
 *     // or using a function that will make the message dynamic.
 *     var maxlength = [9, function(){ return 'The value of path `{PATH}` (`{VALUE}`) exceeds the maximum allowed length ({MAXLENGTH}).'}];
 ...
 *     // or using a function that will make the message dynamic.
 *     var match = [ /\.html$/, function(){ return "That file doesn't end in .html ({VALUE})"} ];
